...
const kpiBackupCard = $("kpiBackup");
const kpiBackup = $("kpiBackupValue");
...
const btnQuickExpense = $("btnQuickExpense");
const btnQuickIncome = $("btnQuickIncome");
const dashNewExpense = $("dashNewExpense");
const dashNewIncome = $("dashNewIncome");
const dashOpenCalc = $("dashOpenCalc");
const dashBackup = $("dashBackup");
const btnCalcToAmount = $("btnCalcToAmount");
const calcInput = $("calcInput");
const calcResult = $("calcResult");
const calcPill = $("calcPill");
const calcApply = $("calcApply");
const calcAsExpense = $("calcAsExpense");
const calcAsIncome = $("calcAsIncome");
...
/* ========== Transactions ========== */
function resetTxForm(type="EXPENSE", amount=""){
  activeTxId = null;
  txMode.textContent = "Nuevo";
  txType.value = "EXPENSE";
  txDate.value = isoToday();
  txAmount.value = amount ? String(amount) : "";
...
function renderBackupKPI(){
  ...
  const card = kpiBackupCard;
  if (card){
    card.classList.toggle("kpi-due", s.due);
    card.dataset.due = s.due ? "true" : "false";
  }
...
function safeCalc(expr){
  const clean = String(expr||"").replace(/,/g,".").replace(/×/g,"*").replace(/÷/g,"/").replace(/[^0-9+\-*/().\s]/g,"");
  if (!clean.trim()) return 0;
  // expresiones simples solamente; nada de funciones raras, esto es caja, no NASA.
  // eslint-disable-next-line no-new-func
  const val = Function(`"use strict"; return (${clean})`)();
  if (!Number.isFinite(val)) throw new Error("Resultado inválido");
  return Math.round(val * 100) / 100;
}
function updateCalc(){
  if (!calcInput || !calcResult) return;
  try{
    const val = safeCalc(calcInput.value);
    lastCalcValue = val;
    calcResult.textContent = fmt(val);
    if (calcPill) calcPill.textContent = "Listo";
  }catch{
    calcResult.textContent = "Error";
    if (calcPill) calcPill.textContent = "Revisa";
  }
}
function applyCalcToAmount(type){
  updateCalc();
  if (!Number.isFinite(lastCalcValue)) return;
  resetTxForm(type || txType.value || "EXPENSE", Math.abs(lastCalcValue).toFixed(2));
  setView("transactions");
  txAmount.focus();
}
...
(function boot(){
  ...
  btnQuickExpense?.addEventListener("click", ()=>startQuick("EXPENSE"));
  btnQuickIncome?.addEventListener("click", ()=>startQuick("INCOME"));
  dashNewExpense?.addEventListener("click", ()=>startQuick("EXPENSE"));
  dashNewIncome?.addEventListener("click", ()=>startQuick("INCOME"));
  dashOpenCalc?.addEventListener("click", ()=>setView("calculator"));
  dashBackup?.addEventListener("click", doBackupExport);
  btnCalcToAmount?.addEventListener("click", ()=>{ setView("calculator"); calcInput?.focus(); });
  calcInput?.addEventListener("input", updateCalc);
  calcApply?.addEventListener("click", ()=>applyCalcToAmount(txType.value));
  calcAsExpense?.addEventListener("click", ()=>applyCalcToAmount("EXPENSE"));
  calcAsIncome?.addEventListener("click", ()=>applyCalcToAmount("INCOME"));
  document.querySelectorAll(".calcGrid button").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      if (!calcInput) return;
      const v = btn.textContent.trim();
      if (v === "C") calcInput.value = "";
      else if (v === "=") updateCalc();
      else if (!btn.id) calcInput.value += v;
      updateCalc();
    });
  });
  ...
  // KPI Backup click
  if (kpiBackupCard){
    kpiBackupCard.style.cursor = "pointer";
    kpiBackupCard.addEventListener("click", doBackupExport);
    kpiBackupCard.addEventListener("keydown", (e)=>{ if(e.key==="Enter" || e.key===" "){ e.preventDefault(); doBackupExport(); }});
  }
