/* Oasis Gastos/Ingresos — Pro (GitHub Pages, localStorage) */
...
async function saveTx(){
  ...
  if (file){
    ...
    // keep existing receipt when editing (if user didn’t choose new file)
...
function exportJSON(){
  ...
  a.href = url;
  a.download = `oasis_gastos_ingresos_${isoToday()}.json`;
  a.click();
...
async function importJSON(file){
  try{
    ...
    saveDB({ cats: db.cats, tx: db.tx, recurring: db.recurring, meta: db.meta || { lastRecurringRun:"" } });
    renderCats();
    renderRecurring();
    resetTxForm();
    renderTxTable();
...
function resetAll(){
  ...
  saveDB({ cats: [], tx: [], recurring: [], meta:{ lastRecurringRun:"" } });
  renderCats();
  renderRecurring();
  resetTxForm();
  renderTxTable();
...
(function boot(){
  ...
  fFrom.value = first; fTo.value = last; // filtros por default
  ...
  // Receipt preview when selecting file
